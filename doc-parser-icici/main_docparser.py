...
from src.docquery import document, pipeline
...
def predict(inp, func, ocr_engine):
  #  img2 = np.asarray(inp)
    cv2.imwrite("dummy.jpg", inp)  
    
    
    doc = document.load_document("dummy.jpg", ocr_engine)
    im = [inp]
    ...
    if "Object detection" in func and "doc_classifier" in func:
        if pred == "Salary Slip":
                     ...
                     for idx, (name,x1,y1,x2,y2,conf) in enumerate(zip(p["name"],p["xmin"],p["ymin"],p["xmax"],p["ymax"],p["confidence"])):
                         ...
                         df1_transposed = get_ocr(crop_img)
...
gr.Interface(fn=predict, 
             ...
             inputs=[gr.Image(), gr.CheckboxGroup(["doc_classifier", "layoutLM", "Object detection"]),  gr.Radio(["paddlepaddle", "easyocr", "microsoft"])],
